...
def extract_c_api(header: Path) -> str:
    ...
    for line in lines:
        ...
        # #define constants
        if stripped.startswith("#define") and not stripped.startswith("#define ZIG_CRYPTO_H"):
            sections.append(f"```c\n{stripped}\n```\n")
            current_comment = []
            continue
...
def main():
    ...
    header = ROOT / "include" / "zig_crypto.h"
    c_content = extract_c_api(header)
    (DOCS / "c-ffi.md").write_text(
        f"# C FFI Reference\n\n"
        f"Auto-generated from [`include/zig_crypto.h`](https://github.com/Jesssullivan/zig-crypto/blob/main/include/zig_crypto.h).\n\n"
        f"{c_content}\n"
    )
    ...
    (DOCS / "zig-api.md").write_text(
        f"# Zig API Reference\n\n"
        f"Auto-generated from Zig source files in [`src/`](https://github.com/Jesssullivan/zig-crypto/tree/main/src).\n\n"
