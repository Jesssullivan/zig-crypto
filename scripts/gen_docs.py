...
Usage:
  ...
  python scripts/gen_docs.py --root-metadata # also refresh legacy root metadata

Root AGENTS/llms surfaces in this repo are curated planning and operator
documents. They are intentionally not regenerated by default.
"""
...
def infer_project_name() -> str:
    """Infer the public project name without depending on checkout path."""
...
def _annotation_from_zig(path: Path) -> str:
    ...
    inferred = {
        ...
        "notify_macos": "macOS notification backend",
...
# ---------------------------------------------------------------------------
# 4. Legacy root metadata generators (opt-in)
# ---------------------------------------------------------------------------

def generate_llms_txt() -> str:
    """Generate legacy uppercase LLMS.txt following https://llmstxt.org/ format."""
    lines = [f"# {REPO_NAME}", ""]
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Generate MkDocs source files from repository source.",
    )
    parser.add_argument(
        "--root-metadata",
        action="store_true",
        help="also regenerate legacy root LLMS.txt and AGENTS.md",
    )
...
def main() -> None:
    ...
    if args.root_metadata:
        # These root files are curated in normal repo work; this opt-in mode is
        # retained for explicit legacy refreshes only.
        write_file(REPO_ROOT / "LLMS.txt", generate_llms_txt())
        write_file(REPO_ROOT / "AGENTS.md", generate_agents_md())
    else:
        print("  skipped root metadata (use --root-metadata to refresh legacy files)")
